...
def mock_request_manager():
    with patch.object(RequestManager, '_instance', None), \
         patch('services.github_user_api.RequestManager') as MockRequestManager:

        mock_instance = MagicMock()
        MockRequestManager.return_value = mock_instance

        mock_instance.get.return_value = MagicMock(status_code=200, json=lambda: {"login": "testuser", "id": 123})
        mock_instance.patch.return_value = MagicMock(status_code=200, json=lambda: {"login": "testuser", "id": 123})
        ...
        mock_instance.post.return_value = MagicMock(status_code=201, json=lambda: {"emails": ["a@b.com"]})
...
def test_get_authenticated_user_with_custom_headers(user_api, mock_request_manager):
    ...
    mock_request_manager.get.assert_called_once_with('https://api.github.com/user', headers=custom_headers)
...
def test_update_profile(user_api, mock_request_manager):
    ...
    mock_request_manager.patch.assert_called_once_with('https://api.github.com/user', json=payload)
    assert response.status_code == 200
...
def test_get_user(user_api, mock_request_manager):
    ...
    mock_request_manager.get.assert_called_once_with('https://api.github.com/users/octocat')
    assert response.status_code == 200
...
def test_follow_user(user_api, mock_request_manager):
    ...
    mock_request_manager.put.assert_called_once_with('https://api.github.com/user/following/testuser')
...
def test_unfollow_user(user_api, mock_request_manager):
    ...
    mock_request_manager.delete.assert_called_once_with('https://api.github.com/user/following/testuser')
    assert response.status_code == 204
...
def test_check_following(user_api, mock_request_manager):
    ...
    mock_request_manager.head.assert_called_once_with('https://api.github.com/user/following/testuser')
    assert response.status_code == 204
